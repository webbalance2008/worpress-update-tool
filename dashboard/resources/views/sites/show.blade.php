...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center space-x-3">
            ...
            <form method="POST" action="{{ route('sites.sync', $site) }}">
                @csrf
                <button type="submit" class="bg-gray-100 text-gray-700 px-3 py-1 rounded text-sm hover:bg-gray-200">
                    Sync Now
                </button>
            </form>
        </div>
            ...
            @if($latestHealthCheck)
                <div class="mt-4 pt-4 border-t border-gray-100">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                        ...
                        @endswitch
                    ">
                        {{ $latestHealthCheck->status->label() }}
                    </span>
    ...
    @if($recentErrors->isNotEmpty())
        <div class="mt-6 bg-white rounded-lg shadow p-6">
            <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Recent Errors</h3>
            <div class="space-y-2">
                @foreach($recentErrors as $error)
                    <div class="flex items-start space-x-3 text-sm">
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                            @switch($error->severity->value)
                                @case('critical') bg-red-100 text-red-800 @break
                                @case('error') bg-red-50 text-red-700 @break
                                @case('warning') bg-yellow-50 text-yellow-700 @break
                                @default bg-gray-50 text-gray-600
                            @endswitch
                        ">
                            {{ $error->severity->value }}
                        </span>
                        <span class="text-gray-700 flex-1">{{ Str::limit($error->message, 120) }}</span>
                        <span class="text-xs text-gray-400 whitespace-nowrap">{{ $error->created_at->diffForHumans() }}</span>
                    </div>
                @endforeach
            </div>
        </div>
